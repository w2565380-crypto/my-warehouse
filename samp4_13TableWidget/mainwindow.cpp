...
void MainWindow::on_actionSetStudentList_triggered()
{
    ...
    QStringList headerText;
    headerText << "学号" << "姓名" << "性别" << "行政班级" << "院系" << "专业" << "修读性质";
    ui->tableInfo->setHorizontalHeaderLabels(headerText);

    // 设置表格行数（5行：前两行 + 当前行 + 后两行）
    ui->tableInfo->setRowCount(5);

    // 模拟从PDF获取的数据 - 这里需要根据实际PDF内容替换
    // 假设您的学号是202105002，取前两行和后两行
    QStringList studentData[5] = {
        {"2023414290228", "苏童鼎", "男", "2023软件2班", "计算机学院", "计算机科学与技术", "必修"},
        {"2023414290229", "孙斯琦", "女", "2023软件2班", "计算机学院", "计算机科学与技术", "必修"},
        {"2023414290231", "翁圳欣", "男", "2023软件2班", "计算机学院", "计算机科学与技术", "必修"},
        {"2023414290232", "吴智攀", "男", "2023软件2班", "计算机学院", "计算机科学与技术", "必修"},
        {"2023414290236", "杨泽仁", "男", "2023软件2班", "计算机学院", "计算机科学与技术", "必修"}
    };

    // 填充表格数据
    for (int i = 0; i < 5; i++) {
        // 学号
        QTableWidgetItem *item = new QTableWidgetItem(studentData[i][0]);
        item->setTextAlignment(Qt::AlignHCenter | Qt::AlignVCenter);
        ui->tableInfo->setItem(i, 0, item);

        // 姓名
        item = new QTableWidgetItem(studentData[i][1]);
        item->setTextAlignment(Qt::AlignHCenter | Qt::AlignVCenter);
        ui->tableInfo->setItem(i, 1, item);

        // 性别
        item = new QTableWidgetItem(studentData[i][2]);
        item->setTextAlignment(Qt::AlignHCenter | Qt::AlignVCenter);
        ui->tableInfo->setItem(i, 2, item);

        // 行政班级
        item = new QTableWidgetItem(studentData[i][3]);
        item->setTextAlignment(Qt::AlignHCenter | Qt::AlignVCenter);
        ui->tableInfo->setItem(i, 3, item);

        // 院系
        item = new QTableWidgetItem(studentData[i][4]);
        item->setTextAlignment(Qt::AlignLeft | Qt::AlignVCenter);
        ui->tableInfo->setItem(i, 4, item);

        // 专业
        item = new QTableWidgetItem(studentData[i][5]);
        item->setTextAlignment(Qt::AlignLeft | Qt::AlignVCenter);
        ui->tableInfo->setItem(i, 5, item);

        // 修读性质
        item = new QTableWidgetItem(studentData[i][6]);
        item->setTextAlignment(Qt::AlignHCenter | Qt::AlignVCenter);
        ui->tableInfo->setItem(i, 6, item);
    }

    // 设置自己学号和姓名的单元格格式（红色粗体）
    // 假设您的数据在第3行（索引为2）
    int myRow = 2; // 根据实际情况调整

    // 设置学号单元格格式
    QTableWidgetItem *studentIDItem = ui->tableInfo->item(myRow, 0);
    if (studentIDItem) {
        QFont font = studentIDItem->font();
        font.setBold(true);
        studentIDItem->setFont(font);
        studentIDItem->setForeground(QBrush(Qt::red));
    }

    // 设置姓名单元格格式
    QTableWidgetItem *nameItem = ui->tableInfo->item(myRow, 1);
    if (nameItem) {
